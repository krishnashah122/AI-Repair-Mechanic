...
## Function to load Gemini model and get responses related to aircraft repair
def get_gemini_response(damage_type, damage_strength, damage_part):
    model = genai.GenerativeModel('gemini-pro')

    # Pre-defined prompt to focus the chatbot on aircraft damage repair
    mechanic_prompt = (
        "You are a professional aircraft repair mechanic. The aircraft has the following damage: "
        "Please provide the possible risks that can occur due to the " f"{damage_type} damage, with a severity of {damage_strength}% on the {damage_part}. Also, suggest some solutions/precautions that should be followed. Please provide me step-by-step process to fix or remove " f"{damage_type} on a aircraft? What are the required tools for it? Please provide a detailed cost estimation for fixing or removing " f"{damage_type} on a aircraft?"
        "Make sure the response is only about aircraft damage repair."
...
st.set_page_config(page_title="Aircraft Repair Mechanic - Text Response Demo")
...
st.header("AI Aircraft Repair Mechanic")
...
damage_part = st.text_input("Enter the part of the aircraft that is damaged (e.g., jet engine, wings, cockpit, etc.):", key="damage_part")

submit = st.button("Ask the Mechanic")

## If the button is clicked, get a response related to aircraft repair
if submit:
    if damage_type.strip() and damage_part.strip():  # Ensure inputs are provided
        ...
        st.write("Please provide all the details about the aircraft damage to get an analysis.")
